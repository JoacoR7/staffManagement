...
const ReciboPage = () => {
  ...
  const cargarEmpleados = async () => {
    try {
      ...
      manejarError(err.message || 'Error al cargar empleados')
    }
  }
  ...
  const cargarItems = async () => {
    try {
      ...
      manejarError(err.message || 'Error al cargar items')
    }
  }
  ...
  const guardar = async () => {
    try {
      ...
      manejarError(err.data?.error || err.message || 'Error al guardar el recibo de sueldo')
    }
  }
  ...
  const renderTable = () => (
    <CCard className="mb-4 shadow-sm">
      <CCardHeader className="d-flex justify-content-between align-items-center">
        ...
        <CButton color="primary" onClick={abrirCrear}>Generar Nuevo Recibo</CButton>
      </CCardHeader>
      <CCardBody>
        <CTable striped hover responsive>
          <CTableHead>
            <CTableRow>
              ...
              <CTableHeaderCell>Mes al que corresponde el pago</CTableHeaderCell>
      ...
      <CCardBody>
        {cargandoOpciones ? (
          ...
          <>
            ...
            <div className="mb-3">
              <CFormLabel>Mes al que corresponde el pago</CFormLabel>
              <CFormInput
                type="number"
                min={1}
                max={12}
                placeholder="Numero de mes (1-12)"
                disabled={soloLectura}
